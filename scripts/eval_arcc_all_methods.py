...
def parse_arc_label(example):
    ...
    if answer.isdigit():
        ...
        if 0 <= idx < len(labels):
            return idx
    raise ValueError(f"Could not parse ARC answer key {answer!r} from labels {labels!r}")


@torch.no_grad()
def evaluate_arcc(runner, examples, max_length):
    runner.model.eval()
    runner.reset_stats()
    correct = 0
...
def make_row(policy, target_saved, realized_saved, arcc):
    return {
        "policy": policy,
        "target_saved": target_saved,
        "realized_saved": realized_saved,
        "arc_challenge_acc": arcc[0],
        "arc_challenge_correct": arcc[1],
        "arc_challenge_total": arcc[2],
    }


def evaluate_runner(runner, target_saved, arcc_examples, args):
    arcc = evaluate_arcc(runner, arcc_examples, args.max_length)
    realized = arcc[3] if runner.mode in {"mod", "gateskip"} else target_saved
    return make_row(runner.policy, target_saved, realized, arcc)
...
def print_summary(rows):
    ...
    print("| policy | target_saved | realized_saved | ARC-C acc | correct/total |")
    print("|---|---:|---:|---:|---:|")
    for row in rows:
        print(
            f"| {row['policy']} | {row['target_saved']:.4f} | {row['realized_saved']:.4f} | "
            f"{row['arc_challenge_acc']:.4f} | {row['arc_challenge_correct']}/{row['arc_challenge_total']} |"
        )


def evaluate_gated_family(rows, arcc_examples, args, config):
    ...
    rows.append(evaluate_runner(MethodRunner("full_model", model, tokenizer, saved_compute=0.0), 0.0, arcc_examples, args))
...
def evaluate_mod_family(rows, arcc_examples, args, config):
...
def evaluate_gateskip_family(rows, arcc_examples, args, config):
...
def evaluate_adaskip_family(rows, arcc_examples, args, config):
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate all methods on ARC-Challenge only.")
    ...
    parser.add_argument("--output-csv", default="outputs/arcc_all_methods.csv")
    ...
    arcc_examples = load_arcc(args.arcc_samples)

    rows = []
    evaluate_gated_family(rows, arcc_examples, args, config)
    evaluate_mod_family(rows, arcc_examples, args, config)
    evaluate_gateskip_family(rows, arcc_examples, args, config)
    evaluate_adaskip_family(rows, arcc_examples, args, config)

    write_csv(rows, args.output_csv)
    print_summary(rows)
    print(f"\nSaved ARC-Challenge evaluation table to {args.output_csv}")
