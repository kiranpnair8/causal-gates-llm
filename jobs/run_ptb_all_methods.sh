#!/bin/bash
#SBATCH --job-name=ptb-all-methods
#SBATCH --output=logs/ptb_all_methods_%j.out
#SBATCH --error=logs/ptb_all_methods_%j.err
...
python scripts/eval_ptb_all_methods.py \
    ...
    --max-length 512 \
    --seed 123 \
    --output-csv outputs/ptb_all_methods.csv
